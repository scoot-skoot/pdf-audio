...
class JobWorker:
    """Background worker that claims jobs and drives the shared PDF→MP3 pipeline."""
    ...
    @classmethod
    def process(cls, conn, job_id, pdf_path, mode, trim_matter):
        last = {"status": "EXTRACTING"}

        def on_event(stage):
            status = cls.STAGE_TO_STATUS.get(stage)
            if status and status != last["status"]:
                last["status"] = status
                cls.set_status(conn, job_id, status)
        ...
        location = Storage.publish(result["output_path"], job_id)
        cls.set_status(conn, job_id, "COMPLETED", result_location=location)
